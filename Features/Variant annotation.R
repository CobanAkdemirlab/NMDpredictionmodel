...
#filter the potential confounding
#### removal of  the single exon genes

df <- df[-which(df$exon_count==1),]
#### remove the low expression genes in whole blood
setwd('~/Desktop/NMDrevisioncode')
...
df <- df[-rem.ind,]
#### remove the egenes
whlbld.egenes <- read.table('/home/iegab/TOPMed2026/Whole_Blood.v8.egenes.txt',sep='\t',header=TRUE)
...
save(whlbld.egenes.sub,file='/home/iegab/TOPMed2026/gnomAD_whlbld.egenes.sub.RData')
