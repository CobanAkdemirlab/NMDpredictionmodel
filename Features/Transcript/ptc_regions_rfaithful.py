...
def _utr3_list(db, tx):
    """
    Faithful derivation of 3'UTR like the R pipeline:
    - Use annotation features if present (above)
    - Else derive as exon segments *after* the CDS terminus (strand-aware)
    """
...
def _first_ejc_downstream(db, tx, ptc_pos):
    """Return (chrom, junction_pos, strand) for the first exon–exon junction downstream of PTC."""
    exons = _exon_list(db, tx)
    if not exons or len(exons) < 2: return None
    strand = tx.strand
    if strand == '+':
        for a, b in zip(exons, exons[1:]):
            if a.start <= ptc_pos <= a.end:
                return (tx.chrom, b.start, strand)
            if ptc_pos < a.start:
                return (tx.chrom, b.start, strand)
        return None
    else:
        for a, b in zip(reversed(exons), reversed(exons[:-1])):
            if a.end >= ptc_pos >= a.start:
...
def _build_regions_for_variant(genome, db, idx, chrom_raw, pos, txnames):
    ...
    if utr3:
        utr3_pieces = [(chrom, u.start, u.end, strand) for u in utr3]
    ...
    # 2) newUTR (PTC to next junction inside CDS) + first200
    cds_list = _cds_list(db, tx)
    if cds_list:
        ejc = _first_ejc_downstream(db, tx, pos)
        if ejc:
            _, jpos, _ = ejc
            pieces = []
            for cds in cds_list:
                if strand == '+':
                    ...
                    if e < pos: continue
                    if s <= jpos <= e:
                        e = jpos; pieces.append((chrom, s, e, strand)); break
                    ...
                    if s > pos: continue
                    if s <= jpos <= e:
                        s = jpos; pieces.append((chrom, s, e, strand)); break
                    pieces.append((chrom, s, e, strand))
            seq_newutr_all = _pieces_to_seq(genome, pieces)
            if seq_newutr_all:
                out["newutr_all"] = seq_newutr_all
                # slice first200 of newUTR (transcript-forward)
                # convert to region-like for reuse
                reg_like = []
                for (_, s, e, _) in pieces:
                    reg_like.append(type(cds_list[0])(
                        seqid=tx.chrom, source="custom", featuretype="tmp",
                        start=s, end=e, score=".", strand=strand, frame=".", attributes={}
                    ))
                first200 = _slice_firstN(reg_like, toward="start", bp=200)
                if first200:
                    seq_newutr_200 = _pieces_to_seq(genome, [(chrom, r.start, r.end, strand) for r in first200])
                    if seq_newutr_200:
                        out["newutr_200"] = seq_newutr_200

    # 3) ±100 windows
    out["ptc_pm100"] = _extract_seq(genome, chrom, pos-100, pos+100, strand)
    ...
    if ejc:
        ...
        if strand == '+':
            ...
            s, e = jpos, pos
        out["ptc_to_ejc"] = _extract_seq(genome, chrom, s, e, strand)
...
class PTCPipelineRFaithful:
    REGION_ORDER = ["utr3_all","utr3_200","newutr_all","newutr_200","ptc_pm100","ejc_pm100","ptc_to_ejc"]
    ...
    def build_regions(self):
        ...
        errors = []
        with Pool(N_CORES, initializer=_init_worker, initargs=(str(self.genome_file), self.db_path)) as pool:
            for idx, seqdict, errs in pool.imap_unordered(_variant_worker, args, chunksize=200):
    ...
    def run_fimo(self):
        ...
        self.fimo_by_region = {}
        base_out = self.workdir / "fimo_regions"
    ...
    def build_matrices(self):
        ...
        outdir = self.workdir / "topmed_matrices"; outdir.mkdir(exist_ok=True)
        full.to_csv(outdir / "motif_presence.csv", index_label="variant_idx")
        print("Saved matrix ->", outdir / "motif_presence.csv")
        self.matrix = full
