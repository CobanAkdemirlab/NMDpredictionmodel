...
# FIX (B1): sequences cached by the previous version are wrong for
# minus-strand transcripts, so the cache now uses a new table name and
# old cached sequences are never reused.
SEQ_CACHE_TABLE = "sequence_cache_v2"


def cds_offset_in_transcript(strand, cds_regions, exon_regions):
    """FIX (B1): length of the spliced 5'UTR, i.e. the number of transcript
    (5'->3') nucleotides before the first CDS nucleotide.

    Regions are 1-based closed genomic intervals. On the + strand the CDS
    starts at min(CDS start) and the 5'UTR lies at lower coordinates; on the
    - strand the CDS starts at max(CDS end) and the 5'UTR lies at HIGHER
    coordinates. The previous code always counted the lower-coordinate
    side, which on the - strand is the 3'UTR.
    """
    if not cds_regions or not exon_regions:
        return 0
    offset = 0
    if strand == '-':
        cds_first_nt = max(end for _, end in cds_regions)
        for exon_start, exon_end in exon_regions:
            if exon_start > cds_first_nt:
                offset += exon_end - exon_start + 1
            elif exon_start <= cds_first_nt <= exon_end:
                offset += exon_end - cds_first_nt
    else:
        cds_first_nt = min(start for start, _ in cds_regions)
        for exon_start, exon_end in exon_regions:
            if exon_end < cds_first_nt:
                offset += exon_end - exon_start + 1
            elif exon_start <= cds_first_nt <= exon_end:
                offset += cds_first_nt - exon_start
    return offset
...
class TranscriptDatabase:
    ...
    def _create_tables(self):
        ...
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {SEQ_CACHE_TABLE} (
                transcript_id TEXT PRIMARY KEY,
                cds_sequence TEXT,
    ...
    def add_transcript(self, transcript_id: str, gene_symbol: str, chromosome: str, 
        ...
        # FIX (B1): strand-aware offset
        cds_start_position = cds_offset_in_transcript(strand, cds_regions, exon_regions or [])
    ...
    def add_sequence(self, transcript_id: str, cds_sequence: str, cds_offset: int = 0):
        ...
        cursor = self.conn.cursor()
        cursor.execute(f"""
            INSERT OR REPLACE INTO {SEQ_CACHE_TABLE} 
    ...
    def get_sequence(self, transcript_id: str) -> Optional[Tuple[str, int]]:
        ...
        for tid in [transcript_id, base_id]:
            cursor.execute(f"SELECT cds_sequence, cds_offset FROM {SEQ_CACHE_TABLE} WHERE transcript_id = ?", (tid,))
            row = cursor.fetchone()
            if row:
...
class TranscriptAnalyzer:
    ...
    def extract_cds_sequence(self, transcript_id: str) -> Optional[Tuple[str, int]]:
        ...
        # FIX (B1): recompute rather than trust the stored value, which the
        # previous version computed incorrectly for minus-strand transcripts
        # (matters when an old --db is reused without --build-db).
        cds_start_pos = cds_offset_in_transcript(strand, cds_regions, exon_regions)
    ...
    def extract_kozak_sequence_at_position(self, sequence: str, aug_position: int) -> Dict:
        ...
        if len(kozak_sequence) >= 10:
            ...
            # FIX (B2): aug_nt_pos is the index of the A of AUG (= +1), so +4
            # is aug_nt_pos + 3 (previously +4, i.e. the +5 nucleotide).
            pos_plus4_idx = (aug_nt_pos + 3) - kozak_start
            ...
            # FIX (B2): removed the '+1C' term. It read index aug_nt_pos + 3,
            # which is the +4 nucleotide, so it gave a point for C at +4.
    ...
    def extract_original_start_kozak(self, sequence: str, cds_offset: int = 0) -> Dict:
        ...
        if cds_offset > 0:
            ...
            # FIX (B2): +4 is cds_offset + 3 (A of ATG is at cds_offset)
            pos_plus4_idx = cds_offset + 3 - kozak_start
            if 0 <= pos_plus4_idx < len(kozak_sequence):
                pos_plus4 = kozak_sequence[pos_plus4_idx]
                if pos_plus4 == 'G':
                    kozak_score += 3
            ...
            # FIX (B2): removed the '+1C' term (it tested the +4 nucleotide for C).
